...
var init_layout_945094a6 = __esm({
  ".svelte-kit/output/server/chunks/__layout-945094a6.js"() {
    ...
    Header = create_ssr_component(($$result, $$props, $$bindings, slots) => {
      let themeText = "";
      return `<header class="${"flex bg-green-400 dark:bg-green-800 dark:text-gray-300 p-1"}"><h1 class="${"mr-4"}"><a sveltekit:prefetch href="${"/top"}">svelte-hn-reader</a></h1>
	<nav class="${"flex flex-grow space-x-1"}"><a sveltekit:prefetch href="${"/top"}">top</a> <span>|</span>
		...
		<button>${escape(themeText)}</button>
		</nav></header>`;
    });
    _layout = create_ssr_component(($$result, $$props, $$bindings, slots) => {
      return `<div class="${"flex flex-col min-h-screen w-11/12 xl:w-10/12 mt-2 mx-auto"}">${validate_component(Header, "Header").$$render($$result, {}, {}, {})}
...
function createNullItem(id, index) {
  return {
    id,
    by: "null",
    url: "",
...
function createUndefinedItem(id, index) {
  return {
    id,
    by: "undefined",
    url: "",
...
function getTimeAgo(time) {
  const sub = new Date().getTime() - time * 1e3;
  const minutes = Math.round(sub / 6e4);
  let a = minutes;
  let b = "m";
  if (minutes >= 518400) {
...
var init_ItemSummary_b22373cc = __esm({
  ".svelte-kit/output/server/chunks/ItemSummary-b22373cc.js"() {
    ...
    ItemSummary = create_ssr_component(($$result, $$props, $$bindings, slots) => {
      ...
      return `<div class="${"wrapper"}"${add_attribute("id", item.id.toString(), 0)}>
	<div class="${"flex flex-col space-x-0"}"><div><a${add_attribute("href", item.url, 0)} class="${"text-lg text-black dark:text-gray-200 visited:text-gray-500 dark:visited:text-gray-400"}">${escape(item.title)}</a>
			${item.url ? `<span class="${"text-gray-500 dark:text-gray-400 text-sm"}">(${escape(getBaseURL(item.url))})</span>` : ``}</div>
		<div class="${"text-gray-500 dark:text-gray-400 text-xs"}">${escape(item.score)} points by ${escape(item.by)}
			${escape(getTimeAgo(item.time))}
			${item.kids ? `| <a sveltekit:prefetch href="${"/item/" + escape(item.id)}">${escape(item.descendants)} comments</a>` : `| <a sveltekit:prefetch href="${"/item/" + escape(item.id)}">comments</a>`}</div>
		${showText && item.text ? `<div><!-- HTML_TAG_START -->${item.text}<!-- HTML_TAG_END --></div>` : ``}</div></div>`;
    });
  }
});
...
async function load3({ page }) {
  ...
  return {
    props: { parentItem: item, importedItems: items }
  };
}
...
var init_id_a2a06508 = __esm({
  ".svelte-kit/output/server/chunks/[id]-a2a06508.js"() {
    ...
    ItemDetail = create_ssr_component(($$result, $$props, $$bindings, slots) => {
      ...
      let expandText = " - ";
      let { next } = $$props;
      let { nextAncestor } = $$props;
      let children = new Array();
      let { item } = $$props;
      if ($$props.next === void 0 && $$bindings.next && next !== void 0)
        $$bindings.next(next);
      if ($$props.nextAncestor === void 0 && $$bindings.nextAncestor && nextAncestor !== void 0)
        $$bindings.nextAncestor(nextAncestor);
      if ($$props.item === void 0 && $$bindings.item && item !== void 0)
        $$bindings.item(item);
      return `<div${add_attribute("id", item.id.toString(), 0)} class="${"mt-4"}"><div class="${"text-gray-500 dark:text-gray-400"}">
		${escape(item.by === void 0 ? "deleted" : item.by)}
		${escape(getTimeAgo(item.time))}

		<a href="${"#" + escape(item.parent)}">parent</a>
		${next !== null || nextAncestor !== null ? `<a href="${"#" + escape(next ?? nextAncestor)}">next</a>` : ``}
		<button>[${escape(expandText)}]</button></div>

	${`<p class="${"max-w-4xl"}"><!-- HTML_TAG_START -->${item.text === void 0 ? "deleted" : item.text}<!-- HTML_TAG_END --></p>`}</div>



${item.kids ? `<div class="${"ml-8"}">${children.length === 0 ? `<p>Loading...</p>` : `${each(children, (child, i) => `${i + 1 < children.length ? `${validate_component(ItemDetail, "svelte:self").$$render($$result, {
        item: child,
        next: children[i + 1].id,
        nextAncestor: children[i + 1].id
      }, {}, {})}` : `${validate_component(ItemDetail, "svelte:self").$$render($$result, {
        item: child,
        next: nextAncestor,
        nextAncestor
      }, {}, {})}`}`)}`}</div>` : ``}`;
    ...
    U5Bidu5D = create_ssr_component(($$result, $$props, $$bindings, slots) => {
      ...
      if ($$props.importedItems === void 0 && $$bindings.importedItems && importedItems !== void 0)
	...
	${each(items, (item, i) => `<div class="${"dark:text-gray-300"}">${i + 1 < items.length ? `${validate_component(ItemDetail, "ItemDetail").$$render($$result, {
      ...
      }, {}, {})}` : `${validate_component(ItemDetail, "ItemDetail").$$render($$result, { item, next: null, nextAncestor: null }, {}, {})}`}
		</div>`)}` : `<span>Error retrieving item: ${escape(errorMessage)}</span>`}`;
    });
  }
});
...
async function load4({ page, fetch: fetch2 }) {
  ...
  return { props: { storyIds: ids, category } };
}
...
<div class="${"flex flex-col overflow-x-hidden"}">${errorMessage ? `<div class="${"font-bold text-2xl text-red-800"}">${escape(errorMessage)}</div>` : ``}

	${each(stories, (story, i) => `<div id="${escape(story.id) + "_scroll"}" class="${"flex space-x-2 items-baseline mb-2"}"><span class="${"min-w-2 text-xl text-gray-400 text-right"}">${escape(i + 1)}.</span>
			${validate_component(ItemSummary, "ItemSummary").$$render($$result, { item: story, showText: false }, {}, {})}
		</div>`)}

	<div class="${"flex-grow"}"></div>
	${showMoreButton ? `<button class="${"pt-2 pb-4 text-left text-gray-500"}">Load more...</button>` : ``}</div>`;
    });
  }
});
    ...
    manifest = {
      ...
      routes: [
        ...
        },
        {
          type: "page",
          pattern: /^\/([^/]+?)\/?$/,
          params: (m) => ({ category: d(m[1]) }),
          a: ["src/routes/__layout.svelte", "src/routes/[category].svelte"],
          b: [".svelte-kit/build/components/error.svelte"]
        }
      ]
    };
